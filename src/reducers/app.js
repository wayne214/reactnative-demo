...
export default (state = initState, action) => {
	...
	switch (action.type) {
		case ActionTypes.ACTION_CHANGE_TAB:
			if (action.payload === 'carriage'){newState = newState.set('shouldEntrustOrderListRefresh', true)}
			if (action.payload === 'order') {newState = newState.set('shouldOrderListRefresh', true)};
		...
		console.log('lqq--payload-',action.payload);
