...
const regs = {
	...
	passwd: /^(?![\d]+$)(?![a-zA-Z]+$)(?![!#$%&*]+$)[\da-zA-Z!#$%&*]{6,24}$/,
		// 6 - 32 字母，数字和符号任意两种以上组合
	newpwd: /^(?![\d]+$)(?![a-zA-Z]+$)(?![!#$%&*]+$)[\da-zA-Z!#$%&*]{6,32}$/,
	...
	companyName: /^[\(\)\（\）\u4E00-\u9FA5]+$/,
	//座机号+手机号（3-8；4-7；手机号）
	phoneNumber: /^(\d{3}-)?\d{8}$|^(\d{4}-)?\d{7}$|^(1[34578]{1})+\d{9}$/,
	//纳税人识别号（15-20位）可以为字母和数字
	taxpayerCode: /^[\d]{15,20}|[\da-zA-Z]{15,20}|[a-zA-Z]{15,20}$/,
